...
def log_initial_data(new_data, type_: str, member_id: int, time_stamp: date):
    sql_database.database.log.log_data(member_id=member_id, log_type=type_, log_date=time_stamp, old_data="NULL",
                                       new_data=new_data)
...
def log_data(reference_data: list, output, time_stamp: date, table_type: TableTypes) -> None:
    match table_type:
...
def _log_member(reference_data: list, output: dict, time_stamp: date) -> None:
